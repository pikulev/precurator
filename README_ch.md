...
- `runtimeRegistry` 提供系统运行时所需的内容：observer、comparator、verifier、tool、model、summarizer 与 checkpointer。
...
`ControlSystemConfig` 存储的是诸如 `observerRef`、`verifierRef`、`comparatorRef`、`toolRefs` 和 `modelRef` 这样的引用。
...
- 如果你的领域需要更丰富的规划或效果执行，你需要通过自己的 observer、comparator、verifier 与 runtime tool 显式建模。
...
- `"increment-observer"` 只是一个 registry key。配置保存的是引用，而 runtime registry 会把它映射到真实的 handler。
- observer 每次加 `2`，只是为了让推进过程保持确定性，并且一眼就能看明白。
...
- `resume(..., { current: { value: 5 } })` 用来演示由操作员提供的状态修正。它展示的是人工干预 API，而不是建议你用手工覆盖来替代正常 observation。
...
3. 在 `RuntimeRegistry` 中绑定真实的 observer、verifier、tool、model 与 checkpointer；
...
- 现有 harness 代码通常可以挂在 `observerRef`、`toolRefs` 或 `verifierRef` 后面；
