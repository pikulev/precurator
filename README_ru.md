...
- `evolve` обновляет `current`. На практике этот шаг может просто читать среду, а может совмещать effect + observe: вызвать tools, продвинуть систему вперед и вернуть следующий `current`.
- `compare` вычисляет структурированное рассогласование между `target` и новым `current`: `errorVector`, `errorScore`, `deltaError`, `errorTrend` и при необходимости `prediction`.
- `verify` независимо решает, можно ли засчитать прогресс. Именно здесь цикл остается в `optimizing`, сходится, останавливается с ошибкой или уходит в `awaiting_human_intervention`.
- `compactMemory` сжимает prompt-facing память до следующей итерации, чтобы reasoning видел ограниченное и нормализованное окно истории, а не бесконечный transcript.

Из этого следуют две важные вещи:

- внутри нет скрытого planner node с "секретной" оркестрацией;
- если в вашем домене нужны отдельные стадии планирования, tool execution или эффекторной логики, вы моделируете их явно внутри `evolve`, `verifier` и runtime tools, а не ждете, что библиотека добавит скрытый orchestration layer.
